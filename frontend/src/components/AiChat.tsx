...
export default function AiChat() {
  ...
  return (
    <div className="h-[400px] flex flex-col bg-black rounded-xl border border-gray-800 mt-6 overflow-hidden">
      <div className="p-4 border-b border-gray-800 flex items-center gap-2 text-aligo-500 bg-zinc-900/40">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
            {msg.role === 'ai' && (
              <div className="w-8 h-8 rounded-full bg-aligo-600/20 flex items-center justify-center text-aligo-500 shrink-0">
                <Bot className="w-4 h-4" />
              </div>
            )}
            <div className={`max-w-[80%] p-3 rounded-lg text-sm ${msg.role === 'user' ? 'bg-aligo-700 text-white rounded-tr-none' : 'bg-gray-800 text-zinc-200 rounded-tl-none border border-gray-700'}`}>
              {msg.text}
            </div>
            {msg.role === 'user' && (
              <div className="w-8 h-8 rounded-full bg-aligo-600/20 flex items-center justify-center text-aligo-500 shrink-0">
      ...
      <div className="p-3 border-t border-gray-800 bg-black">
        <form onSubmit={handleSend} className="flex gap-2">
          <input
            ...
            className="flex-1 bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-aligo-600 focus:ring-1 focus:ring-aligo-600 text-zinc-200 disabled:opacity-50"
          />
          <button 
            type="submit" 
            className="bg-aligo-700 hover:bg-blue-700 text-white p-2 rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center min-w-8" 
